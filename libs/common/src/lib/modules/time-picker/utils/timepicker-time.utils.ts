import { forceZero } from '../../../functions/dateFromTime/dateFromTime'
  ...
  if (config.min || config.max) {
    return hours.map(value => {
      const hour =
        config.format === 24
          ? value.time
          : TimeAdapter.formatHour(value.time, config.format, config.period)

      const currentTime = `${forceZero(hour % 12)}:00 ${
        hour > 11 || hour === 0 ? 'pm' : 'am'
      }`

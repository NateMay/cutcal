...
describe('Calendar Controls Component', () => {
  ...
  it('applys the date format', () => {
    ...
    parent.monthBtnDisabled = true;
    fixture.detectChanges();
    expect(getEl(fixture, '.non-btn-month-label').textContent).toContain('Mar 19');

    parent.format = 'MMM yyyy';
    parent.monthBtnDate = new Date(2018, 3, 17);
    parent.monthBtnDisabled = false;
    fixture.detectChanges();
    expect(getEl(fixture, '[id^=controls-month-btn]').textContent).toContain('Apr 2018');
  });
  ...
  it('apply labels', () => {
    ...
    fixture.detectChanges();
    expect(getDe(fixture, '[id^=prev-month]').attributes['aria-label']).toBe('my prev btn');
    expect(getDe(fixture, '[id^=next-month]').attributes['aria-label']).toBe('my next btn');
  });

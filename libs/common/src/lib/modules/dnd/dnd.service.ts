...
export class DndSvc implements OnDestroy {
  ...
  get ghost(): HTMLImageElement {
    ...
    this.renderer.setAttribute(this._ghost, 'src', '../../../../assets/svgs/meal.svg');
    return this._ghost;
  }
  ...
  constructor(
    ...
    fromEvent(this.document, 'mousedown').pipe(
      tap((mousemove: MouseEvent) => this._lockImage = true)
    ).subscribe();

    fromEvent(this.document, 'mouseup').pipe(
      tap((mousemove: MouseEvent) => this._lockImage = false)
    ).subscribe();

...
export class GoogleService {
  ...
  getWikiDetails(searchTerm: string, itemNum: number = 0): Observable<WikiDetails> {
    const modified = searchTerm.trim().replace(/\s\s+/g, ' ').replace(' ', '+');
    return this.http.get(`${this.endPoint}?key=${this.apiKey}&cx=${this.engine}&q=${modified}`).pipe(
